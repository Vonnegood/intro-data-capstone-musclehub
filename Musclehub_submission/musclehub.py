...
plt.pie(ab_counts.gender.values, labels=['A','B'], autopct='%.0f%%')
plt.axis('equal')
...
app_pivot= app_counts.pivot_table(index='ab_test_group', columns='is_application', values='gender').reset_index()
print app_pivot
...
Y = [[200, 50], [250, 75]]
bchi2, bpval, bdof, bexpected =chi2_contingency(Y)
print bpval
...
Z = [[200, 2304], [250, 2250]]
cchi2, cpval, cdof, cexpected =chi2_contingency(Z)
print cpval
...
ax.set_xticklabels(['Fitness Test','No Fitness Test'])
plt.title('Percent who Applied and then Purchased')
plt.savefig('Applied_and_purchased.png')
plt.show()
...
plt.savefig('visit_who_purchased.png')
plt.show()
